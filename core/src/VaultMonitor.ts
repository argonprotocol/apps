...
export class VaultMonitor {
  ...
  public readonly mainchain: ArgonClient;
  ...
  constructor(
    ...
    this.blockWatch.events.on('vaults-updated', (header, vaultIds) => this.onVaultsUpdated(header.hash, vaultIds));
  ...
  public async monitor(justPrint = false) {
    ...
    console.log(
      `${justPrint ? 'Run' : 'Started'} at block ${blockHeader.number.toNumber()} - ${blockHeader.hash.toHuman()}`,
    );
  ...
  private async recheckAfterActive(vaultId: number) {
    ...
    if (this.shouldLog) {
      console.log(`Waiting for vault ${vaultId} to activate ${activationDate.toLocaleString()}`);
    }
  ...
  private async onVaultsUpdated(blockHash: Uint8Array, vaultIds: Set<number>) {
    await this.reloadVaultsAt([...vaultIds], blockHash).catch(err => {
      console.error(`Failed to reload vault ${[...vaultIds].join()} at block ${u8aToHex(blockHash)}:`, err);
    });
    this.printVaults();
  }
