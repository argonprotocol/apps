...
import { createArgonClient, NetworkConfig, NetworkConfigSettings } from '@argonprotocol/apps-core';
...
(async () => {
  ...
  for (const [name, config] of Object.entries(NetworkConfigSettings)) {
    ...
    try {
      ...
      const client = createArgonClient(await Promise.race([
  ...
  fs.writeFileSync(
    Path.join(dirname, 'network.config.json'),
    JSON.stringify(NetworkConfigSettings, null, 2) + '\n',
    'utf-8',
  );
