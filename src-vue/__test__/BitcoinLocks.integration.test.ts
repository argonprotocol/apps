  ...
  it('accepts a mismatch candidate and persists the accepted funding record on chain and in the db', async () => {
    ...
    try {
      ...
      try {
        ...
        expect(harness.bitcoinLocks.shouldShowFundingMismatch(accepted.lock)).toBe(false);
      } finally {
        stopTracking();
      }
    } finally {
    ...
    try {
      ...
      try {
        ...
        await waitFor(30e3, 'live mismatch return resume stays pending', async () => {
          ...
          if (harness.bitcoinLocks.shouldShowFundingMismatch(resumed)) return;
          if (harness.bitcoinLocks.getCurrentMismatchOrphanReturn(resumed.utxoId!, returned.candidate)) return;
  ...
  it('keeps a new funded lock isolated after a prior release and a prior mismatch return', async () => {
    ...
    try {
      ...
      try {
        ...
        expect(harness.bitcoinLocks.getCurrentMismatchOrphanReturn(fundedThird.lock.utxoId!)).toBeUndefined();
...
async function observeMismatchCandidate(
  ...
  const observed = await waitFor(
    ...
    async () => {
      ...
      const candidate = harness.bitcoinLocks.getPreferredMismatchCandidate(currentLock);
      if (!candidate) return;
      if (!candidate.firstSeenOnArgonAt) return;
      if (!harness.bitcoinLocks.shouldShowFundingMismatch(currentLock)) return;
...
async function returnMismatchAndWaitForReadyToResume(
  ...
  expect(harness.bitcoinLocks.canReturnMismatchCandidate(observed.lock, observed.candidate)).toBe(true);
  expect(harness.bitcoinLocks.canAcceptMismatchCandidate(observed.lock, observed.candidate)).toBe(true);
  ...
  await waitFor(30e3, 'live mismatch return tracked on argon', () => {
    ...
    const record = harness.bitcoinLocks.getCurrentMismatchOrphanReturn(refreshed.utxoId!, observed.candidate);
    if (!record) return;
...
async function acceptMismatchFunding(
  ...
  await waitFor(30e3, 'mismatch accept argon progress', () => {
    progress.updateLock(getCurrentLock(harness, observed.lock.utxoId!));
    if (progress.mismatchAcceptArgon.value.confirmations < 0) return;
    if (progress.mismatchAcceptArgon.value.expectedConfirmations <= 0) return;
    return true;
  });
...
async function returnExpiredMismatchAndWaitForChainRestore(
  ...
  const expired = await waitFor(
    ...
    async () => {
      ...
      const currentCandidate =
        harness.bitcoinLocks.getPreferredMismatchCandidate(currentLock) ??
        harness.bitcoinLocks
          .getMismatchCandidates(currentLock)
          .find(candidate => candidate.txid === observed.candidate.txid && candidate.vout === observed.candidate.vout);

      if (!harness.bitcoinLocks.isFundingWindowExpired(currentLock)) return;
      if (!currentCandidate) return;
      if (!harness.bitcoinLocks.canReturnMismatchCandidate(currentLock, currentCandidate)) return;
      if (
        ![
  ...
  await waitFor(30e3, 'mismatch return tracked on argon', () => {
    ...
    progress.updateLock(refreshed);
    const record = harness.bitcoinLocks.getCurrentMismatchOrphanReturn(refreshed.utxoId!, expired.candidate);
    if (!record) return;
    if (record.status !== BitcoinUtxoStatus.ReleaseIsProcessingOnArgon) return;
    if (progress.orphanedReturnArgon.value.confirmations < 0) return;
    if (progress.orphanedReturnArgon.value.expectedConfirmations <= 0) return;
  ...
  const completed = await waitFor(
    ...
    async () => {
      ...
      const chainLock = await BitcoinLock.get(chainClient, refreshed.utxoId!);
      if (chainLock) return;
      if (harness.bitcoinLocks.shouldShowFundingMismatch(refreshed)) return;
