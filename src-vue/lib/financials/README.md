...
Bond flexibility events are read into detached facts and published by `ArgonBonds` only after a complete historical pass. The domain also owns finalized-event reconciliation and a durable block cursor. Existing bond rows have unconfirmed flexibility history until that pass finishes; an empty timeline alone does not prove a lot was never flexible. A return stays unavailable while its required timeline is unconfirmed; active bond and owning-vault returns also wait for a startup or failed finalized-event catch-up. Completed bonds with confirmed release history retain their returns during that catch-up. Current principal and vault value remain visible.
...
- **Bonds:** Each treasury bond lot is a position, although the UI can combine them. ARGN principal remains at face value; ARGNOT principal uses its current or release mark for net worth. Bond income return is distributed earnings divided by the principal's ARGN value when locked, excluding later ARGNOT price movement. A bond into the user's own vault remains an account asset and its ordinary-period earnings count toward account RTD. A flexibility change closes one bond-return period and starts another; the period boundaries are stored with the bond's financial history. Flexible-period returns are attributed to the operator vault, so only those periods are excluded from the bond and account return calculations. Flexible principal contributes timing to the vault's return but does not change its displayed asset value.
- **Bitcoin locks:** One position follows a lock across ratchets. Live BTC and pending mint use the current mark, while received liquidity excludes pending and burned mint. Liquid RTD accumulates the liquidity unlocked by upward ratchets and the liability pocketed by downward ratchets, then deducts the transaction and net securitization fees recorded by its own events; live BTC prices and hypothetical close quotes do not change it. A completed Liquid freezes that realized return using its recorded redemption. A completed release separately settles the underlying Bitcoin asset from its removal-time BTC mark, recorded redemption, Argon and Bitcoin fees, compensation, and removal time. Until settlement recovery completes, the observable value remains visible but its return is unavailable. An expired unspent UTXO is not presented as a live lock or returned BTC.
