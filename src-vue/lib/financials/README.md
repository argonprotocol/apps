...
- **Stable swaps:** Ethereum ARGN is one position marked at the current pool price. Purchase history supplies cost basis only when it is complete and reconciles to the wallet quantity; otherwise its value remains visible but its return is unavailable.
