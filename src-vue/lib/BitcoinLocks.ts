...
export default class BitcoinLocks {
  ...
  private async submitOrphanReleaseToBitcoin(
    ...
    try {
      ...
      const existingTxStatus = await this.#mempool.getTxStatus(txid, this.oracleBitcoinBlockHeight).catch(() => undefined);
      if (existingTxStatus) {
      ...
      const bitcoinTxid = await this.#mempool.broadcastTx(hexTx);
      const mempoolTip = await this.#mempool.getTipHeight().catch(() => this.oracleBitcoinBlockHeight);
