...
const props = withDefaults(
  defineProps<{
    data: [ISlice, ISlice, ISlice, ISlice];
...
function render() {
  ...
  const sanitized: [ISlice, ISlice, ISlice, ISlice] = [
    { ...props.data[0], value: clampNonNeg(props.data[0].value) },
    { ...props.data[1], value: clampNonNeg(props.data[1].value) },
    { ...props.data[2], value: clampNonNeg(props.data[2].value) },
    { ...props.data[3], value: clampNonNeg(props.data[3].value) },
  ];

  const total = sanitized.reduce((sum, slice) => sum + slice.value, 0);

  // If all slices are 0, show a single grey circle.
  ...
  const fallback = ['#8a1d85', '#d7c3d7', '#6f7fd1', '#89d0bd'];
