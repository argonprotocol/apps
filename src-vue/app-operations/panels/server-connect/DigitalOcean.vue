      ...
      <div Browser class="flex h-full w-full flex-col rounded-md border border-slate-800/20 shadow">
        ...
        <div class="relative mx-px grow overflow-hidden rounded-b">
          ...
          <div v-else-if="currentStep === 1.4" class="absolute top-px left-0 flex h-full w-full flex-col bg-[#EAECF1]">
            ...
            <img
              src="../../../assets/digitalocean-1.4-message.png"
              class="relative -top-[10%] z-10 mx-auto w-[39.8%]"
            />
...
import { IConfigServerAddDigitalOcean } from '../../../interfaces/IConfig.ts';
...
const hasApiKey = !!config.serverAdd?.digitalOcean?.apiKey;
...
const apiKey = Vue.ref(config.serverAdd?.digitalOcean?.apiKey ?? '');
...
async function connect(): Promise<IConfigServerAddDigitalOcean> {
  if (!apiKey.value) {
