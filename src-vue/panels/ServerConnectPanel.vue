            ...
            <div class="flex h-full w-full flex-col px-4">
              <h2
                ...
                >
                  ...
                  <TabsTrigger
                    data-testid="ServerConnectPanel.selectedTab='local'"
                    class="hover:text-argon-700/60 data-[state=active]:text-argon-600 flex flex-1 cursor-pointer items-center justify-center rounded-tl-md px-5 leading-none outline-none select-none data-[state=active]:font-bold"
                    value="localComputer">
                    Local Computer
                  </TabsTrigger>
              ...
              <div class="mx-4 flex flex-row justify-end space-x-4 rounded-b-lg border-t border-slate-300 py-4">
                <div v-if="serverAddError" class="flex grow items-center rounded-md bg-red-200 p-2 pl-4">
                  <div class="flex">
                    <div class="shrink-0">
...
import {
  IConfigServerAdd,
  IConfigServerAddCustomServer,
  IConfigServerAddDigitalOcean,
  IConfigServerAddLocalComputer,
} from '../interfaces/IConfig.ts';

export interface IServerConnectChildExposed {
  connect: () => Promise<IConfigServerAddDigitalOcean | IConfigServerAddLocalComputer | IConfigServerAddCustomServer>;
}
</script>
...
basicEmitter.on('openServerConnectPanel', async () => {
  isOpen.value = true;
  isLoaded.value = true;
  serverAddError.value = '';

  if (config.serverAdd?.localComputer) {
...
async function connect() {
  ...
  try {
    config.serverAdd = await extractServerAdd();
    config.resetField('serverInstaller');
    ...
    serverAddError.value = error.message;
    throw error;
  } finally {
...
async function extractServerAdd(): Promise<IConfigServerAdd> {
  if (selectedTab.value === 'digitalOcean') {
    return { digitalOcean: (await digitalOceanRef.value?.connect()) as IConfigServerAddDigitalOcean };
  }
  if (selectedTab.value === 'localComputer') {
    return { localComputer: (await localComputerRef.value?.connect()) as IConfigServerAddLocalComputer };
  }
  return { customServer: (await customServerRef.value?.connect()) as IConfigServerAddCustomServer };
}
