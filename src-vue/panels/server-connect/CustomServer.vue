...
import { IServerConnectChildExposed } from '../ServerConnectPanel.vue';
...
const sshUser = Vue.ref(config.serverAdd?.customServer?.sshUser ?? '');
const ipAddressAndMaybePort = Vue.ref(config.serverAdd?.customServer?.ipAddress ?? '');
if (config.serverAdd?.customServer?.port && config.serverAdd?.customServer?.port !== 22) {
  ipAddressAndMaybePort.value = `${ipAddressAndMaybePort.value}:${config.serverAdd?.customServer?.port}`;
}
...
async function connect(): Promise<IConfigServerAddCustomServer> {
  ...
  const serverAdd: IConfigServerAddCustomServer = {
    port,
