           ...
           {{ update ? 'Update Available' : 'Checking for Updates' }}
            ...
            <div class="px-6 text-base pb-3 mb-6 relative z-10 grid grid-col gap-2">
              <p class="mb-2">
                ...
                <template v-else>
                  You are already on the latest version. No updates available.
                </template>
              </p>
...
async function relaunchApp() {
  try {
    console.log(
      `Relaunching app to install new version (upgrading ${update.value?.currentVersion} to ${update.value?.version})`,
    );
