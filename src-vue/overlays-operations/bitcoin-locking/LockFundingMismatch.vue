...
const mismatchCandidates = Vue.computed(() => bitcoinLocks.getMismatchCandidates(currentLock.value));
const preferredCandidate = Vue.computed(() => bitcoinLocks.getPreferredMismatchCandidate(currentLock.value));
const mismatchCandidateCount = Vue.computed(() => mismatchCandidates.value.length);

const orphanedRecord = Vue.computed(() => {
  const utxoId = currentLock.value.utxoId;
  if (!utxoId) return undefined;
  return bitcoinLocks.getCurrentMismatchOrphanReturn(utxoId, preferredCandidate.value);
});
const candidateForDisplay = Vue.computed(
  () => preferredCandidate.value ?? mismatchCandidates.value[0] ?? orphanedRecord.value,
);

const satoshisObserved = Vue.computed(() => {
  return candidateForDisplay.value?.satoshis ?? bitcoinLocks.getReceivedFundingSatoshis(currentLock.value) ?? 0n;
});

const observedSatoshis = Vue.computed(() => satoshisObserved.value ?? 0n);
const differenceSatoshis = Vue.computed(() => observedSatoshis.value - currentLock.value.satoshis);
...
const isFundingExpired = Vue.computed(() => bitcoinLocks.isFundingWindowExpired(currentLock.value));
const isFundingReadyToResume = Vue.computed(() => bitcoinLocks.isFundingReadyToResumeStatus(currentLock.value.status));

const canReturnMismatch = Vue.computed(() => {
  const candidate = preferredCandidate.value;
  if (!candidate) return false;
  return bitcoinLocks.canReturnMismatchCandidate(currentLock.value, candidate);
});

const canAcceptMismatch = Vue.computed(() => {
  const candidate = preferredCandidate.value;
  if (!candidate || mismatchCandidateCount.value > 1 || isFundingExpired.value) return false;
  return bitcoinLocks.canAcceptMismatchCandidate(currentLock.value, candidate);
});

const acceptTxInfo = Vue.computed(() => {
  const candidate = preferredCandidate.value;
  if (candidate) return bitcoinLocks.getMismatchAcceptTxInfo(currentLock.value, candidate);
...
const showOptionCards = Vue.computed(() => {
  return (
    !showOrphanProgress.value &&
    !showReturnCompleteState.value &&
    !showResumeFundingState.value &&
    !acceptInProgress.value &&
    !acceptCompleted.value &&
...
const persistentMismatchError = Vue.computed(() => bitcoinLocks.getMismatchError(currentLock.value));
