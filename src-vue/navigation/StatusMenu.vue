<template>
  <div ref="rootRef" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave">
    <DropdownMenuRoot :openDelay="0" :closeDelay="0" class="pointer-events-auto relative" v-model:open="isOpen">
      <DropdownMenuTrigger Trigger class="cursor-pointer focus:outline-none">
        <div
          ...
          <div
            ...
            <div
              ...
              <div v-if="miningStatus === 'Funded'" class="rounded-md bg-slate-400/10 px-6 py-5">
                ...
                <p class="mt-px py-1 opacity-80">
                  ...
                  argon{{
                    microgonToArgonNm(wallets.totalMiningMicrogons).formatIfElse(
                      '< 100',
                      '0,0.[000000]',
                      '0,0.[00]',
                    ) === '1'
                ...
                <div class="mt-1 flex flex-row items-center gap-x-3">
                  <div class="text-argon-600 cursor-pointer" @click="openFundMiningAccountOverlay">
                    Add More Capital
                  </div>
              ...
              <div v-else-if="miningStatus === 'Underfunded'" class="rounded-md bg-red-100/70 px-4 py-4">
                ...
                <p class="mt-px py-1 opacity-80">
                  ...
                  <template v-if="miningMicronotsNeeded > 0n">
                    ...
                    argonot{{
                      micronotToArgonotNm(miningMicronotsNeeded).formatIfElse('< 100', '0,0.[000000]', '0,0.[00]') ===
                      '1'
                        ? ''
                        : 's'
                    }}
            ...
            <template v-if="config.hasSavedVaultingRules">
              <div v-if="vaultingStatus === 'Funded'" class="rounded-md px-4 py-4">
                ...
                <p class="mt-px py-1 opacity-80">
                  Your vaulting account has enough funds to operate. No further action is required at this time.
                  However, you can always improve your vault with more funds.
                </p>
                <div class="mt-2 flex flex-row gap-x-2">
                  <button
