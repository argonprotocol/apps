...
function createBitcoinFundLockMismatchOperation(): Operation<IBitcoinFlowContext, IFundLockMismatchState> {
  const operation = new Operation<IBitcoinFlowContext, IFundLockMismatchState>(import.meta, {
    async inspect({ flow }) {
      ...
      const mismatchPending = panelState.chainState.phase !== 'none' || ui.mismatchPanelVisible;
    ...
    async run({ flow, flowName, input, state: flowState }) {
      ...
      await flow.poll<IFundLockMismatchState>(
        latest => {
        ...
        },
        {
          pollMs: 1_000,
          timeoutMs: 180_000,
          timeoutMessage: `${flowName}: mismatch funding did not advance into the mismatch phase.`,
        },
      );
    },
  });
