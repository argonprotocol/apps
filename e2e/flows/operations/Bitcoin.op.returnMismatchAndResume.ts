...
export default new Operation<IBitcoinFlowContext, IReturnMismatchAndResumeState>(import.meta, {
  async inspect({ flow, state: flowState }) {
    ...
    const returnInProgress = panelState.chainState.hasOrphanedReturnInProgress;
  ...
  async run({ flow, flowName }, state) {
    ...
    await pollEvery(
      ...
      async () => {
        ...
        if (latest.chainState.hasOrphanedReturnInProgress) {
    ...
    await pollEvery(
      ...
      async () => {
        ...
        return (
          ...
          !latest.chainState.mismatchRequired &&
          !latest.returnPanelVisible &&
          !latest.acceptVisible
        );
      },
      {
        timeoutMs: 180_000,
