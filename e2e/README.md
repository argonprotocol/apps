...
# 6) Capture screenshots during flow execution (saved under /tmp by default)
...
- `E2E_SCREENSHOT_MODE` controls automatic capture:
  ...
  - `interaction` (per-command screenshots + failures)
- `E2E_SCREENSHOT_DIR` overrides output directory (default: `/tmp/e2e-screenshots`).
